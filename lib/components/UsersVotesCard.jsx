...
export const UsersVotesCard = () => {
  ...
  console.log(tokenHolder, delegate)
  if (!usersAddress || tokenHolderIsLoading || delegateLoading || (!tokenHolder.hasBalance && !tokenHolder.hasDelegated)) {
    return null
  }
  ...
  return <Banner className='mb-4'>
    <h4 className='font-normal mb-4'>Total votes</h4>
    <div className='flex flex-col sm:flex-row'> 
      <h2 className={classnames('leading-none mr-4', {
...
const DelegateTrigger = props => {
  ...
  if (txInFlight?.completed && !txInFlight?.error) {
    return <p className='p-2 rounded bg-light-purple-35 text-green my-auto'>🎉 Successfully activated your votes 🎉</p>
  }
  ...
  if (txInFlight?.completed && txInFlight?.error) {
    return <>
      ...
      <button
        type='button'
        className='underline trans mt-auto'
  ...
  if (txInFlight?.inWallet) {
    return <p className='mt-auto text-green opacity-80'>Please confirm the transaction in your wallet</p>
  }
  ...
  if (!selfDelegated) {
    return <p className='mt-auto'>You have delegated <b>{votes}</b> votes to <b>{tokenHolder.delegate.id}</b></p>
  }
