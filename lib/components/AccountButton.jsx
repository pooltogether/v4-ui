...
export function AccountButton(props) {
  ...
  const ethBalanceNumber = ethBalance && Number(displayAmountInEther(ethBalance, { precision: 2 }))
    ...
    <>
        ...
        <>
          ...
          >
            {pendingTransactionsCount > 0 ? (
              <>
                <span className='text-inverse hover:text-green font-bold'>{pendingTxJsx}</span>
              </>
            ) : (
              <>
                <PoolCountUp start={0} end={ethBalanceNumber} decimals={2} /> ETH
              </>
            )}
          </button>
        </>
      ...
      >
        ...
        >
          {pendingTransactionsCount > 0 && (
            <>
              <div className='block xs:hidden text-green hover:text-inverse'>{pendingTxJsx}</div>
            </>
          )}
