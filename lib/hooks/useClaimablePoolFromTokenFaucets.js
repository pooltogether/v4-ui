...
export const useClaimablePoolFromTokenFaucets = () => {
  const {
    ...
    isFetched: isTokenFaucetAddressesFetched
...
async function getClaimablePoolFromTokenFaucets (provider, usersAddress, tokenFaucetAddresses) {
  try {
    ...
    const total = claimableAmounts.reduce((total, tokenFaucet) => total + tokenFaucet.claimable, 0)

    return { claimableAmounts, total }
  } catch (e) {
    console.error(e.message)
    return { claimableAmounts: [], total: 0 }
  }
}
