...
function useFetchProposals () {
  ...
  const currentBlock = useCurrentBlock(readProvider)

  return useQuery(
    [QUERY_KEYS.proposalsQuery, chainId, currentBlock?.blockNumber],
    async () => {
      return getProposals(readProvider, chainId, currentBlock)
    },
    {
      enabled: !pauseQueries && chainId && readProviderIsLoaded && !isEmpty(currentBlock)
    }
  )
}
  ...
  try {
    ...
    subgraphData.proposals.forEach((proposal) => {
      ...
      const endDateSeconds =
        currentTimestamp + SECONDS_PER_BLOCK * (
          Number(proposal.endBlock) - currentBlockNumber
        )
