...
export function useAccountGovernanceData () {
  const {
    ...
    isFetched,
    error
  } = useAccountGovernanceDataQuery()

  const [accountGovernanceData, setAccountGovernanceData] = useAtom(
    accountGovernanceDataQueryAtom
  )
...
function useAccountGovernanceDataQuery () {
  ...
  return useQuery(
    ...
    {
      enabled: !pauseQueries && chainId && usersAddress && !error,
      refetchInterval
    }
  )
}
...
async function getAccountGovernanceData (chainId, accountAddress) {
  ...
  const variables = {
    accountAddress: '0x7e4a8391c728fed9069b2962699ab416628b19fa'
  }
