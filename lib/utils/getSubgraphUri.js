...
export function getSubgraphUri(chainId, incomingBlockNumber, env = process.env.NEXT_JS_ENV) {
  ...
  if (incomingBlockNumber === -1) {
    ...
    incomingBlockNumber >= v3_0_1StartingBlock &&
    incomingBlockNumber < v3_1_0StartingBlock
  ) {
    version = 'v3_0_1'
    ...
    console.warn(
      'getPoolSubgraphUri(), Could not find Subgraph URI for chainId, env and incomingBlockNumber:',
      chainId,
      env,
      incomingBlockNumber
    )
