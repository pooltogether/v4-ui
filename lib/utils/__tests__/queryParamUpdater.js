...
describe('queryParamUpdater', () => {
  describe('add', () => {
    it('updates a path that has no query', () => {
      ...
      expect(router.push).toHaveBeenCalledWith('/[var]?method=crypto', '/asdf?method=crypto', {
        shallow: true,
      })
    })
  ...
  describe('removeAll', () => {
    it('gets rid of all query params', () => {
      ...
      expect(router.push).toHaveBeenCalledWith('/[var]', '/asdf', { shallow: true })
    })
  })
  ...
  describe('remove', () => {
    it('gets rid of a specific query param', () => {
      ...
      expect(router.push).toHaveBeenCalledWith('/[var]?yes=yo', '/asdf?yes=yo', { shallow: true })
    })
